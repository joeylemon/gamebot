...
const users = Object.keys(ids).filter(key => ids[key].birthday !== undefined).map(key => ids[key])
for (const user of users) {
    const bday = new Date(user.birthday)
    bday.setFullYear(new Date().getFullYear())
    var j = schedule.scheduleJob(bday, function () {
        constants.client.channels.fetch(config.get("discord_channel_id"))
            ...
            .catch(err => console.error(`Cannot find discord_channel_id: ${err}`))
    })
}
